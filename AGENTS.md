...
`bo` mirrors `tree`'s process status convention. The C source keeps a global
`errors` counter for traversal/listing errors, but `tree_main` returns
`errors ? 2 : 0`; the counter is not returned directly.

- `0`: listing completed successfully with no traversal errors; `--help` and `--version` also exit successfully.
- `1`: command/setup failure, such as invalid arguments, missing option arguments, invalid sort/level values, invalid output file, or fatal allocation/setup failure.
- `2`: listing completed but one or more filesystem traversal errors occurred. Multiple missing/unreadable paths still collapse to this fixed status.
